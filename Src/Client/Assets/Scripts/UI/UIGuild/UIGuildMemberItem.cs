...
public class UIGuildMemberItem : ListView.ListViewItem
{
    ...
    {
        this.background.color = selected? Color.blue : Color.white;
    }
    ...
    public void SetGuildMemberInfo(NGuildMemberInfo item)
    {
        ...
        if (this.status != null) this.status.text = this.Info.Status == 1? "Online" : TimeUtil.GetTime(this.Info.lastTime).ToShortDateString();
    }
}
